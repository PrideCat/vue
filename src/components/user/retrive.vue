  ...
  <div id="retrive">
    ...
    <div class="fromBox">
      ...
      >
        ...
        <el-table-column
          prop="money"
          :label="lang.lang=='cn'?'獎金':'Bonus'"
          align="center"
          width="120"
        ></el-table-column>
        <el-table-column
          prop="createTime"
          :label="lang.lang=='cn'?'獎勵時間':'Reward Time'"
          align="center"
          width="200"
        ></el-table-column>
        <el-table-column
          prop="trace"
          :label="lang.lang=='cn'?'狀態':'Status'"
          align="center"
          width="120"
        >
          <template slot-scope="scope">
...
export default {
  ...
  data() {
    ...
    let searchStartDate =
      ...
      "-" +
      getNumber(searchDate.getMonth() + 1) +
      "-" +
      getNumber(searchDate.getDate());
  ...
  methods: {
    ...
    init() {
      ...
      let date = new Date();
      let startDateStrs = search.startDate.split("-");
      let startDate = new Date(
        startDateStrs[0],
        startDateStrs[1] - 1,
        startDateStrs[2]
      );
      let endDateStrs = search.endDate.split("-");
      let endDate = new Date(
        endDateStrs[0],
        endDateStrs[1] - 1,
        endDateStrs[2]
      );
      let OneYearAgo = date.getTime() - 31536000000;
      if (search.type === "-2") delete search.type;
      if (startDate.getTime() < OneYearAgo || endDate.getTime() < OneYearAgo) {
        console.log("選擇範圍不能超過一年前");
        this.$message.error(
          this.lang.lang == "cn"
            ? "選擇範圍不能超過一年前"
            : "Choice cannot exceed one year ago"
        );
        return;
      }
    ...
    daochu() {
      ...
      // },'',1,1);
      // return;
      const tableDom = window.document.getElementsByClassName(
        "el-table__body"
      )[0];
